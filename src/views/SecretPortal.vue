<template>
  <div class="duplication-page">
    <div class="container py-5">
      ...
      <div v-if="isLoggedIn" class="tool-header text-center mb-5">
        <h1 class="tool-title mb-2">Duplication Detection System</h1>
        <p class="text-muted">Upload your transaction or entry spreadsheet to automatically find and report duplicate records.</p>
      </div>
      ...
      <!-- Duplication Detection System Section -->
          ...
          <div class="tool-card">
            ...
            <div class="card-body-tool p-4">
              ...
              <div v-if="!fileLoaded" class="upload-bay text-center p-5" 
                ...
                <h4 class="text-dark">Drag & Drop Excel or CSV File Here</h4>
                <p class="text-muted">Supports .xlsx, .xls, and .csv formats</p>
                <button class="btn btn-primary-custom mt-3" @click="triggerFileInput">
                  <i class="fa-solid fa-folder-open me-2"></i>Browse Files
                </button>
              </div>

              <!-- File Stats & Column Selectors -->
              <div v-else>
                ...
                <div class="results-panel p-4">
                  <div class="results-summary d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
                    ...
                    <div class="d-flex gap-3">
                      <button v-if="duplicateCount > 0" class="btn btn-success" @click="downloadCleanFile">
                        <i class="fa-solid fa-file-shield me-2"></i>Download Clean File
                      </button>
                      <button v-if="duplicateCount > 0" class="btn btn-outline-primary" @click="downloadDuplicatesOnly">
                    ...
                    <div class="table-responsive table-wrap">
                      <table class="table table-bordered table-hover mb-0">
                        <thead class="table-light">
                          <tr>
                            <th scope="col" style="width: 140px;">Row Index(es)</th>
                        ...
                        <tbody>
                          <template v-for="(group, gIdx) in filteredDuplicateGroups" :key="gIdx">
                            <tr v-for="(row, rIdx) in group.rows" :key="rIdx" :class="{ 'group-start': rIdx === 0 }">
                              <!-- Group metadata / row indicators -->
                              <td>
                                <span v-if="rIdx === 0" class="badge bg-danger text-white">Group {{ gIdx + 1 }}</span>
                                <span class="badge bg-secondary ms-1">Row {{ group.indices[rIdx] }}</span>
                              </td>
    ...
    const analyzeDuplicates = () => {
      ...
      parsedRows.value.forEach((row, index) => {
        const key = selectedCols.value
          .map(col => String(row[col] !== undefined ? row[col] : '').trim().toLowerCase())
          .join('|')
